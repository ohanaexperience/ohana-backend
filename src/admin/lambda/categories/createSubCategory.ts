...
export const handler: APIGatewayProxyHandler = async (event) => {
    ...
    try {
        ...
        const { categoryId, name, slug, imageMimeType } = body;

        if (!categoryId || !name || !slug) {
            return {
                statusCode: 400,
                body: JSON.stringify({ message: "categoryId, name and slug are required" }),
            };
        }
        ...
        const result = await controller.createSubCategory({
            authorization,
        ...
        console.error("createSubCategory handler error:", error);
