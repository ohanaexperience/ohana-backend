...
export const getExperiences = middy(async (event: ExperienceSearchData) => {
    ...
    try {
        ...
        const experiences = await db.experiences.searchExperiences(queryParams);
    ...
    .use(zodValidator({ queryStringParameters: ExperienceSearchSchema }))
    .use(cors());
...
export const hostUpdateExperience = middy(
    async (event: UpdateExperienceData) => {
        const { authorization } = event.headers;
        const {
            experienceId,
            title,
        ...
        try {
            ...
            const createdExperience = await db.experiences.update(
                experienceId,
                {
                    hostId: host.id,
                    title,
