...
import {
    GetHostReviewsEvent,
    GetHostReviewsPathSchema,
    GetHostReviewsQuerySchema,
} from "@/reviews/validations/public/getHostReviews";
...
export const handler = middy(async (event: GetHostReviewsEvent) => {
    const userId = event.requestContext.authorizer?.claims?.sub;

    return await reviewController.getPublicHostReviews(
        event.pathParameters.hostId,
